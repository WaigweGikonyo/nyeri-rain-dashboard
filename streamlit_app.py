...
DASHBOARD_URL = "https://nyeri-rain-dashboard-6nvsflctxyimknz7sactb3.streamlit.app/" # <--- Update this after deploying!
...
----------------------------------
Developed by Nyeri Farming Intelligence Unit ❤️
"""
...
with st.sidebar:
    ...
    st.write("Current Recipients:", len(RECEIVERS))
    if st.button("📧 Send Test Alert (Email)"):
        with st.spinner("Sending..."):
            if send_alert_email("DEMO TEST", "The system is checking email delivery. Everything is working!", 0.0):
                st.success("Test Email Sent!")
            else:
                st.error("Failed to send email. Check App Password.")
    st.divider()
    st.info("The dashboard auto-refreshes every 30s and sends emails automatically if the Season Level changes.")

# ───── 5. LOGIC & TRIGGER ─────
if not raw_data:
    st.warning("📡 Station Syncing... Checking Supabase for data.")
...
if current_label != st.session_state.last_notified_label:
    if send_alert_email(current_label, advice, total_rain):
        st.toast(f"📧 Alert Dispatched: {current_label}", icon="✅")
        st.session_state.last_notified_label = current_label
...
with c1:
    ...
    fig.update_layout(title="<b>8-WEEK AI PREDICTION (MM)</b>", template="plotly_dark", 
                      paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=300)
    st.plotly_chart(fig, use_container_width=True)

with c2:
    gauge = go.Figure(go.Indicator(mode="gauge+number", value=total_rain, 
                                  title={'text': "<b>SEASON TOTAL</b>"}, 
                                  gauge={'bar':{'color':color}, 'axis':{'range':[0,500]}}))
