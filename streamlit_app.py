...
with c1:
    ...
    fig = go.Figure(go.Bar(
        x=[f"Wk {i+1}" for i in range(8)], 
        y=forecast, 
        marker_color='#00D4FF',
        opacity=0.8
    ))
    fig.update_layout(
        title={'text': "<b>WEEKLY RAINFALL PREDICTION</b>", 'y':0.95, 'x':0.5, 'xanchor': 'center', 'yanchor': 'top', 'font': {'color': 'white'}},
...
with c2:
    ...
    gauge = go.Figure(go.Indicator(
        ...
        gauge={
            'bar':{'color':color}, 
            'axis':{'range':[0,600], 'tickcolor': "white"},
            'steps': [
                {'range': [0, 250], 'color': 'rgba(255, 82, 82, 0.1)'},
                {'range': [250, 600], 'color': 'rgba(0, 230, 118, 0.1)'}
            ]
        }
    ))
